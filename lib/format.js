...
// A srcset across Cloudinary widths, so a phone stops downloading the 1200px
// negative it then paints into a 340px card. Cloudinary already serves AVIF or
// WebP through f_auto, so this is only about pixel count.
export function productSrcSet(p, widths = [400, 600, 900, 1200, 1600]) {
  return widths.map((w) => `${productImg(p, w)} ${w}w`).join(', ');
}

// The same, for a bare public_id — country heroes and journal covers.
export function srcSetFor(publicId, widths = [600, 900, 1200, 1800]) {
...
// "Türkiye" used to come out as `t-rkiye` — the ü was not in [a-z0-9] so it
// became a hyphen, and that meaningless URL was live and in the sitemap.
// Decomposing to NFD first lets the combining diaeresis be dropped on its own,
// which leaves the base letter behind: turkiye. Everything without an accent is
// unaffected, so no other slug moves.
